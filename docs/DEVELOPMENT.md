...
├─ Jobs/
│  └─ <JobName>/
│     ├─ <JobName>RotationEntry.cs
│     ├─ <JobName>RotationUi.cs
│     ├─ <JobName>Settings.cs
│     ├─ docs/
│     ├─ tests/
│     └─ Resolvers/
...
- 编译时可以直接使用 `HiAuRo.Helper` 提供的各职业静态 API。
...
```csharp
public sealed class ExampleJobRotationEntry : IRotationEntry, ISettingsProvider<ExampleJobSettings>
{
    public string AuthorName { get; } = "Kairo";
    public bool UseCustomUi { get; } = false;
    public IEnumerable<HiAuRo.ACR.Jobs> TargetJobs { get; } = [HiAuRo.ACR.Jobs.Example];
    public ExampleJobSettings Settings { get; set; } = new();

    public Rotation? Build(string settingFolder)
    {
        return new Rotation
        {
            TargetJob = HiAuRo.ACR.Jobs.Example,
            AcrType = AcrType.PvE,
            MinLevel = 1,
            MaxLevel = 100,
            SlotResolvers = [...]
...
builder.AddDropdown("战斗模式", settings.CombatModeOptions, ref settings.CombatMode);
```
...
职业专属开发记录放在 `Jobs/<JobName>/docs/DEVELOPMENT.md`，不要写回顶层总规范。
